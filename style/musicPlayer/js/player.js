        ...
        this.songs = [ 
			{
                ...
                title: '简单的简单的简单的',
                singer: '郁一凡',
                songUrl: '/style/musicPlayer/songs/郁一凡 - 简单的简单的简单的.mp3',
                imageUrl: '/style/musicPlayer/images/songs/郁一凡 - 简单的简单的简单的.jpg'
            },		
			{
                id: 1,
                title: '等风吹',
                singer: '不是花火呀、小田音乐社',
                songUrl: '/style/musicPlayer/songs/不是花火呀、小田音乐社 - 等风吹.mp3',
                imageUrl: '/style/musicPlayer/images/songs/不是花火呀、小田音乐社 - 等风吹.jpg'
            },
		    {
                id: 2,
                title: '风吹过八千里',
                singer: '苏星婕',
                songUrl: '/style/musicPlayer/songs/苏星婕 - 风吹过八千里.mp3',
                imageUrl: '/style/musicPlayer/images/songs/苏星婕 - 风吹过八千里.jpg'
            }
        ]
    }
    //根据索引获取歌曲的方法
...
class PlayerCreator {
    ...
    bindEventListener() {
		...
		//音乐列表
		this.$musicList = new Btns('.player-control__btn--list', {
            click: this.showMusicList.bind(this)
        });
        //禁音
		...
		//播放器切入切出
		this.$hidePlayer = new Btns('.hidePlayer', {
            click: this.hidePlayer.bind(this)
        });
	...
	//播放器切入切出
	hidePlayer(e){
		var playBox = $(".music-player")[0];
		var hidePlayerObject = $(".hidePlayer")[0];
		var offsetLeft = playBox.offsetLeft;
		if(offsetLeft<0){
			$(".music-player").removeClass("slide-out-left");
			$(".music-player").addClass("slide-in-left");
			playBox.style.left = "5px";
			hidePlayerObject.style.background = "url('/style/musicPlayer/images/hidePlayIcon.svg') no-repeat";
		}else{
			$(".music-player").removeClass("slide-in-left");
			$(".music-player").addClass("slide-out-left");
			playBox.style.left = "-442px";
			hidePlayerObject.style.background = "url('/style/musicPlayer/images/showPlayIcon.svg') no-repeat";
		}
	...
	//列表点击显示
	showMusicList(e){
		var list = $(".music-player__list")[0];
		var displayStatus = list.style.display;
		if(displayStatus == 'none'||displayStatus == ''){
			$(".music-player__list").removeClass("fade-out");
			$(".music-player__list").addClass("fade-in-fwd");
			list.style.display = "block";
		}else{
			$(".music-player__list").removeClass("fade-in-fwd");
			$(".music-player__list").addClass("fade-out");
...
//定时隐藏函数
function timingHide(time){
	var playBox = $(".music-player")[0];
	var offsetLeft = playBox.offsetLeft;
	if(offsetLeft > 0 && music_player_status == "blur"){
...
$('.music-player__main').mouseenter(function(){
	music_player_status="focus";
	clearTimeout(timing);
});
$('.music-player__main').mouseleave(function(){
	music_player_status="blur";
	timingHide(7000);
});
	timingHide(10000);
